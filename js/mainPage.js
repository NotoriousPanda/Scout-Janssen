  ...
  rqAPI('https://www.thebluealliance.com/api/v3/event/' + event_key + "/matches", function() {
    ...
    response.json().then(function(value){
      ...
      for(i=0; i < teamData.length; i++){
        blueAllianceKeys = [];
        redAllianceKeys = [];
        packet.push([teamData[i].match_number, teamData[i].alliances.blue.team_keys, teamData[i].alliances.red.team_keys]);
      }
      ...
      for(i=0; i < teamData.length; i++){
        createRow("match" + matchNumbers[i]);
        ...
        createColumn(matchNumbers[i], "match" + matchNumbers[i]);
        createColumn(blueA[0] + ", " + blueA[1] + ", " + blueA[2], "match" + matchNumbers[i]);
        createColumn(redA[0] + ", " + redA[1] + ", " + redA[2], "match" + matchNumbers[i]);
      }
